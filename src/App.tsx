  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-wrap">
        <section className="page-heading"><div><p className="eyebrow"><span /> Monday, September 1, 2026</p><h1>{active === 'Overview' ? <>Good morning, Jose<span>.</span></> : <>{active}<span>.</span></>}</h1><p className="subheading">{active === 'Overview' ? 'Here’s the pulse of your growth engine.' : 'ClubGamerZone growth workspace.'}</p></div><div className="heading-actions"><label className="scope-select"><small>Reporting scope</small><select value={product} onChange={event => setProduct(event.target.value)} aria-label="Reporting scope">{products.map(item => <option key={item}>{item}</option>)}</select></label><label className="scope-select"><small>Date range</small><select value={range} onChange={event => setRange(event.target.value)} aria-label="Date range"><option>Last 7 days</option><option>Last 30 days</option><option>This quarter</option><option>This year</option></select></label><button className="button button-primary" onClick={() => { setActive('Leads & pipeline'); setLeadFormSignal(value => value + 1); }}><Plus size={16} /> Add lead</button></div></section>
        ...
        {active !== 'Overview' ? <WorkspaceModule active={active} product={product} openLeadFormSignal={leadFormSignal} /> : <>
        <section className="metric-grid"><Metric icon={Users} label="Open opportunities" value={stats.opportunities} change="+18.4%" note="vs. previous period" tone="aqua" /><Metric icon={CircleDollarSign} label="Pipeline value" value={stats.pipeline} change="+12.8%" note="weighted: $92.3k" tone="lime" /><Metric icon={Gauge} label="Qualified rate" value={stats.qualified} change="+6.2%" note="of new inquiries" tone="violet" /><Metric icon={ArrowUpRight} label="Cost per qualified lead" value={stats.cpl} change="−14.5%" note="across paid channels" tone="coral" /></section>
        ...
        <section className="dashboard-grid bottom-grid"><div className="panel pipeline-panel"><PanelHeader title="Pipeline" detail="4 active stages" action="Open CRM" /><div className="pipeline-columns">{['New inquiry','Qualified','Proposal','Won'].map((stage, idx) => <div className="pipeline-column" key={stage}><div className="column-head"><span>{stage}</span><b>{idx === 0 ? '12' : idx === 1 ? '6' : idx === 2 ? '4' : '2'}</b></div>{leads.filter(l => l.stage === stage || (idx === 1 && l.stage === 'Discovery')).slice(0, 2).map(lead => <LeadCard key={lead.name} lead={lead} />)}</div>)}</div></div><div className="panel integrations-panel"><PanelHeader title="Account registry" detail="Connection readiness" action="Manage" /><div className="integration-list"><Integration icon={BarChart3} name="Google Analytics 4" detail="clubgamerzone.com" status="Not connected" tone="aqua" /><Integration icon={Target} name="Meta Business" detail="ClubGamerZone" status="Not connected" tone="violet" /><Integration icon={Bot} name="AI assistant" detail="Netlify Function" status="Endpoint pending" tone="lime" /><Integration icon={Globe2} name="Netlify" detail="clubgamerzone-webapp" status="Needs token" tone="coral" /></div><button className="registry-link" onClick={() => setActive('Account registry')}>View all connections <ArrowUpRight size={14} /></button></div></section>
        </>}
